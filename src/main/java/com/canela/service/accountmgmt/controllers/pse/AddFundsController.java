...
public class AddFundsController {
    ...
    @CrossOrigin("*")
    public ResponseEntity<String> addFunds(@PathVariable(value = "accountId") String id, @RequestBody PseRequest req) {
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(null);
    }

    static class PseRequest{
        @Schema(description = "Amount to add", example = "100000")
        private Long amount;
